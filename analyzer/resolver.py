...
class DNSResolver:
    ...
    def inspect_uri(self, name: str) -> UriObservation:
        """URI lookup at <name>. Does not fetch the target or guess prefixes."""
        host = name.rstrip(".").lower()
        try:
            records = self.resolve_uri(host)
        except DomainNotFoundError:
            return evaluate_uri(host, ())
        except DNSQueryError as exc:
            return evaluate_uri(host, (), error=str(exc))
        return evaluate_uri(host, records)

    def resolve_uri(self, name: str) -> list[DNSRecord]:
        return self._query(name, "URI")

    def inspect_dname(self, name: str) -> DnameObservation:
        """DNAME lookup at <name>. Does not synthesize CNAME or walk the subtree."""
        host = name.rstrip(".").lower()
        try:
            records = self.resolve_dname(host)
        except DomainNotFoundError:
            return evaluate_dname(host, ())
        except DNSQueryError as exc:
            return evaluate_dname(host, (), error=str(exc))
        return evaluate_dname(host, records)
    ...
    def inspect_openpgpkey(self, name: str, local_part: str) -> OpenpgpkeyObservation:
        """OPENPGPKEY lookup at hash._openpgpkey.<name>. Does not dump the key."""
        qname = openpgpkey_query_name(name, local_part)
        try:
            records = self.resolve_openpgpkey(qname)
        except DomainNotFoundError:
            return evaluate_openpgpkey(qname, local_part, ())
        except DNSQueryError as exc:
            return evaluate_openpgpkey(qname, local_part, (), error=str(exc))
        return evaluate_openpgpkey(qname, local_part, records)

    def resolve_openpgpkey(self, name: str) -> list[DNSRecord]:
        return self._query(name, "OPENPGPKEY")
