...
def test_uri_missing_is_info_and_unscored() -> None:
    from analyzer.risk import WEIGHTS
    ...
    missing = next(item for item in report.findings if item.code == "uri_missing")
    assert missing.severity == "info"
    assert WEIGHTS["uri_missing"] == 0
    assert report.risk.value == 0


def test_uri_timeout_is_info_unreadable() -> None:
    from analyzer.uri import evaluate_uri

    observation = evaluate_uri("example.com", (), error="DNS query timed out.")
    report = SecurityAnalyzer().analyze(
        _clean_lookup(),
        evaluate_dnssec(dnskey_found=True, ds_found=True, ad_flag=True),
        inspect_spf(_clean_lookup().txt),
    ...
    unread = next(item for item in report.findings if item.code == "uri_unreadable")
    assert unread.severity == "info"
    assert not any(item.code == "uri_missing" for item in report.findings)
    assert report.risk.value == 0
